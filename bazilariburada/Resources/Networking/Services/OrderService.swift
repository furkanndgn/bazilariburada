...
class OrderService: ObservableObject {
    ...
    func placeAnOrder(token: String) {
        orderSubscription = networkManager.performRequest(endpoint: "\(ordersKeywork)", method: .POST,
    ...
    func getUsersAllOrders(token: String) {
        orderSubscription = networkManager.performRequest(endpoint: "\(ordersKeywork)", method: .GET,
                                                          requiresAuthentication: true, token: token)
            .receive(on: DispatchQueue.main)
            .sink(receiveCompletion: networkManager.handleCompletion, receiveValue: { [weak self] response in
